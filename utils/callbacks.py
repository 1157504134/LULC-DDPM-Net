...
class EvalCallback():
    ...
    def get_miou_png(self, image,diffusion,opt):
        #---------------------------------------------------------#
        ...
        with torch.no_grad():
            ...
            #   图片传入网络进行预测
            #---------------------------------------------------#
            feat = get_Tenosr(diffusion=diffusion,opt=opt,image=images)
            pr = self.net(images,feat)[0]
            #---------------------------------------------------#
            #   取出每一个像素点的种类
            #---------------------------------------------------#
    ...
    def on_epoch_end(self, epoch, model_eval,diffuiosn,opt):
        if epoch % self.period == 0 and self.eval_flag:
            ...
            for image_id in tqdm(self.image_ids):
                ...
                #   获得预测txt
                #------------------------------#
                # feat  = get_Tenosr(diffuiosn,opt)
                image       = self.get_miou_png(image,diffuiosn,opt)
