...
from model.cd_modules.sknet import SKConv
...
class AttentionBlock(nn.Module):
    def __init__(self, dim, dim_out):
        ...
        self.block = nn.Sequential(
            ...
            nn.ReLU(),
            SKConv(dim_out,int(dim_out)),
            # SKUnit(in_features=dim, mid_features=int(dim_out/2), out_features=int(dim_out))
...
class cd_head_v2(nn.Module):
    ...
    def __init__(self, feat_scales, out_channels=2, inner_channel=None, channel_multiplier=None, img_size=256, time_steps=None):
        ...
        # Define the parameters of the change detection head 定义变化检测头的参数
        ...
        # Convolutional layers before parsing to difference head 卷积层在解析到差异头之前
        ...
        for i in range(0, len(self.feat_scales)):
            ...
            if i != len(self.feat_scales)-1:
                ...
                print("_______________________________attention_______________")
                print(f"dim:{dim}")
        ...
        # Final classification head 最终分类头
        clfr_emb_dim = 64
